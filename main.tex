...
\begin{equation} 
	\int_{\mathds{R}_{+}^{n-3}} \prod_{i=1}^{n-3}\frac{\dif x_{i}}{x_{i}}x_{i}^{\alpha' X_{i}}\prod_{I}p_{I}(\mathbf{x})^{-\alpha'c_{I}}	\:, \label{int1}
\end{equation}
where $\mathbf{X}:=\{X_{1},\cdots,X_{i-3}\}$ and $c_{I}$ are linear combinations of Mandelstam variables $s_{ij}$'s. A \emph{stringy} integral is a integral of form eq.(\ref{int1}) but with arbitrary substraction-free polynoimals $p_{I}$. 
...
On the one hand, performing Minkowski sum for polytopes analyticly is nearly impossible. On the other hand, to obtain the canonical form $\Omega(N_{P})$ from the pushforward $\Phi_{\ast}\bigl(\bigwedge_{i=1}^{n-3} \dif \log x_{i}\bigr)$ requires to solve highly non-linear equations, just like the case in CHY formalism.
...
\begin{itemize}
	\item Some polynomials $p_I$ other than $x_{i}$ vanishing at the origin (or a vertex).
...
the leading order of the integral will not change.
\end{lem}
...
By using the language cone of a polynomial $p$, the goal of blowing-up is clear.
If one gets cones of a polynomial $p$ like
\begin{center}
...
\]
and it can be visualized by the cone.
...
cones. Let's give a quick review here.

Let's start from a cone $C[p]$. The vertices set of $C[p]$ is denoted by $M_p$. After
...
that if $|M_p|=1$, then $p$ is decoupled. Therefore, a possible winning strategy for A is
to find a set of variables to blow up at each turn such that $|M_p|$ strictly decreases.
...
the number of variables of non-decoupled part of polynomial. At each turn, we only need 
...
\[
	l(v)=\#\{i\,:\, v_j=\max_i v_i\}+\#\{i\,:\, v_j=\min_i v_i\},
\] 
...
Newton Polytope: For a subtraction free Laurent polynomial 
\[
...
\]
Therefore, one can project $\operatorname{im}(\mathbf X)$ by setting some $x_i=1$ so that 
there's no more vector $\mathbf s$ such that $\mathbf s\cdot \mathbf n^I=0$ for all $I$. 
Conversely, one can recover the original image by solving these equations $\mathbf s\cdot\mathbf X=0$}. 
\end{pro}
...
if and only if $\mathbf\Lambda\in (N(p))^\circ$. Moreover, the solution is always unique for 
each interior point.
...
\]
which is direct result from the convexity of $N(p)$: (figure \cite{}) If $\mathbf\Lambda \in (N(p))^\circ$,
for a given $\mathbf y\neq 0$, the hyperplain $L$ with normal vector $\mathbf y$ crossing $\mathbf \Lambda$ 
divides the space $\mathbb R^D$ into semi-spaces $L^+$ and $L^-$, then $\mathbf{n}^I$ in $L^+$ are what we 
are looking for.
...
$F(t\mathbf y)\to 0$ when $t\to \infty$. As a positive convex function, $F(\mathbf y)$ has no saddle point 
in $\mathbb R^D$.
...
We assume that $\alpha_p>0$ for all $p$. Here, $F(\mathbf y)$ is no longer a strict convex function. However,
take a positive number $\alpha_0$ such that $\alpha_0< \alpha_p$ for all $p$, consider a new function
\[
...
so is their product. Since $F(y)^{1/\alpha_0}$ has the same minimums as $F(y)$, so we can further assume that 
$\alpha_p>1$ for all $p$.

Finally, we claim that $F(y)$ does not take minimum when $\mathbf{y}$ goes to infinity along any direction. 
