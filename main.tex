...
to show the general procedure to calculate the leading order fo stringy integrals
by blow-ups.
...
the vertex $(0,0)$ is blowed up to a $1$-dimensional simplex $\mathbb P^1_+=\{[u:v]\,:\,u,v\geq 0\}$ 
...
%We now carefully consider which terms can be thrown away.
...
$C_{I}=(\mathbf n^I+\mathbb R^D_{+})$, the above
...
but terms other than vertices in the Newton polyhedron 
...
\]
We will get the polynomial $p$ again and the endless loop if we always choose such
