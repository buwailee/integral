...
\begin{abstract}
	We provide an efficient method, blowing up, to compute leading order contributions of the recently introduced stringy integrals. The algorithm also gives a new winning strategy of the well-known Hironaka's polyhedra game.
\end{abstract}
\newpage
...
% (\textcolor{red}{C:This short review, especially this example, is a bit confusing and misleading, at least I don't get the point, maybe some pictures or other example can help to clarify it.})
...
where $[y_1,\dots,y_n]$ is the projective coordinate. \textcolor{red}{C: I don't undertand, say what is the range of $i_{j},k,i_{k},j$, what is the dimension of the surface. make it more plain}) These equation can be easily
...
\]
for which the leading contribution come from the neighbourhoods of four vertices $(0,0)$, $(0,\infty)$, $(\infty,0)$ and $(\infty,\infty)$. One can easily see that the vertices $(0,\infty)$, $(\infty,0)$ and $(\infty,\infty)$ are normal crossing. The consequece of this fact is the integrand decouples into powers of $x$ and $y$, then the leading contribution can be trivially obtained, for example 
\begin{align*}
...
The vertex $(0,0)$ is harder due to the behavor of the mixed factor $(x+y+xy)^{-\alpha' c}$ at the neighbourhood of thie vertex. % $(x,y)\to (0,0)$
% \[
...
However, we can always drop the term $xy$ which becomes irrelevant when $(x,y)\to (0,0)$. %because $xy\ll x$, $y$. 
%For the remaining terms $x+y$, 
Then we can introduce new variables $(t,p)$ by the map $x = tp$ and $y=t(1-p)$, \textit{i.e. blow-up}. The vertex (0,0) is blowed up to a unit interval with $t=0$ and $p\in [0,1]$ under this map, and it is easy to see that the factor $(x+y)$ is dominated by $x$ when $p\to 0$ and by $y$ when $p \to1$. By intersering $t$ and $p$, we have
 \begin{equation}\label{canonicalformunderblowup}
	\frac{dxdy}{xy} = \frac{dp}{p(1-p)}\frac{dt}{t},
 \end{equation}
and
\begin{equation*}
	I(0,0)\approx\int_0^\epsilon\frac{dt}{t} t^{\alpha'(a+b-c)}\int_0^{1}\frac{dp}{p(1-p)}p^{\alpha' a}(1-p)^{\alpha' b}%\sim \frac{1}{\alpha'}\frac{1}{a+b-c}\int_0^{1}\frac{dp}{p(1-p)}p^{\alpha' a}(1-p)^{\alpha' b}
...
\draw[->] (-0.3,1) -- (2.8,1);
\draw[->] (-0.2,0.9) -- (-0.2,4);
\node at (-2.8,2) {\small $p\approx 0$};
\node at (-2.2,-2) {\small $p\approx 1$};
\node at (-4,-1) {\small $t$};
...
% For the second example \eqref{exam:2}, the regulating polynomial $p=x^3+y^3+x y$ has a singularity at (0,0). This singularity can be blowed up by setting
...
For the second example \eqref{exam:2}, the regulating polynomial $p=x^3+y^3+x y$ has a singularity at (0,0). This singularity can be blowed up by setting $x=tu$ and $y=tv$, then we can see that the original curve is decomposed into two curves $x^2+y=0$ and $x+y^2=0$ generated by blowing up as in the diagram 
\begin{center}
...
\begin{enumerate}
	...
	\begin{center}
		...
		\draw (-2,-0.5) -- (4,2);
		\draw (3.6,1.3) -- (-2,-0.5);
		\node[fill=green,circle,inner sep=1pt] at (2.4,0.9) {};
		\end{tikzpicture}
	\end{center}
	...
	\begin{center}
		...
		\draw[fill=gray!30] (2.5,0.2) node[fill=red,circle,inner sep=1pt] {}
			...
			-- cycle;
		\node[fill=green,circle,inner sep=1pt] at (3.1,-1.3) {};
		\end{tikzpicture}
		$\longmapsto$
		\begin{tikzpicture}[scale=0.75,baseline={([yshift=-.5ex]current bounding box.center)}]
