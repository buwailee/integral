...
The method is related to the well-known Hironaka's polyhedra game, and the given 
algorithm can be used on similar problems, \textit{e.g.} sector decomposition.
\end{abstract}
\newpage
...
\end{equation}
where $D=n{-}3$, $X_{i}$ and $c_{I}$ are linear combinations of Mandelstam variables $s_{ij}$'s. A \emph{stringy} integral is an integral of form eq.(\ref{int1}) but with arbitrary subtraction-free polynomials $p_{I}$. 
...
Essentially, finding such a series of blow-ups is equivalent to give a winning strategy for the (simplified) Hironaka's polyhedra game.
...
from a geometric viewpoint. The simplified version allow $\mathcal P_1$ to 
...
Before elaborating this algorithm, we first give several simple examples to get a feeling of this blow-up method.
...
In this section, we will give several examples to illustrate this blow-up method, all examples comes from the so-called cluster stringy integrals~\cite{Arkani-Hamed:2019mrd,Arkani-Hamed:2019plo} which are closely related to the origin string integrals. The dimension of all examples in this section is 2 or 3 and the regulating polynomials are simple, so the blow-up prescriptions can be designed \emph{ad hoc} without a universal algorithm.
