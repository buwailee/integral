...
\[
	...
	|z_i-z_j|^{\alpha' s_{ab}}
\]
is defined on a component $\mathcal M_{0,n}^+$ of real points of moduli space of $n$ punctures 
on the sphere. In the language of positive geometry \cite{}, $\mathcal M_{0,n}^+$ is a positive
...
relation between its field theory limit and the polytope given by $\{p_I\}$.
As in the original superstring amplitude, the field theory limit of a stringy integral is 
its leading order as $\alpha'\to 0$.

The main purpose of this article is to show a method to calculate the leading order of 
...
For the first part, when $\alpha'\to 0$, the pole structure of 
...
Let's first consider the simplest case where all $p_I=1$ in the integral eq.\eqref{int1}.
...
\begin{itemize}
	\item There're some polynomials $p_I$ vanishing at a vertex.
	\item Furthermore, the vertex is a singularity of $p_I$.
\end{itemize}
However, they can be overcome by a series of blowing up. 
...
\end{center}
\vspace{-3ex}
Left: $p=x+y+xy$, Right: $p=x^3+y^3+xy$. 
