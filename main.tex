...
\[
	...
	\]
The middle integral is finte when $\alpha'\to 0$ so that it does not contribute, the other two integrals are
	\[
...
There's one important lesson from the above simple integral, the leading terms of the integral come from the vertices of the regions. 

Now let's come to the two dimensional example,
\[
	I=\int_{\mathbb R_+^2} \frac{dx}{x}\frac{dy}{y}x^{\alpha' a}y^{\alpha' b}(x+y)^{-\alpha' c}.
\]
The leading terms come form the neighbourhoods of the vertices, let's conider the simplest vertice first, 
\begin{align*}
...
\end{align*}
similarly, $I(\infty,0)\sim ({\alpha'}^2 b (c-a))^{-1}$. Now the hard vertice
\begin{align*}
...
Now let's consider the general story. Genearlly, the integral near a vertice need not to be decoupled, so blowing-up is necessary. 
...
\]
where $y_n=1-(y_1+\cdots+y_{n-1})$. Therofore,
\[
