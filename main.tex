...
\begin{enumerate}
	\item If the matrix is equivalent to $1\times 1$ matrix $(0)$, then player $A$ wins.
	...
	\item A $1\times N$ matrix is equivalent to the $1\times 1$ matrix $(0)$.
\end{enumerate}
Geometrically, eq.\eqref{linearprog} means that the constant vector $(1,\dots,1)$ 
is in the cone spanned by row vectors $\{n_i^I\,:\,i\in S\}$. It's a standard linear programming
problem. However, $(1,\dots,1)$ is usually not
in this cone. What's more, after blowing up, we delete some vectors, and usually the cone becomes
smaller so that $(1,\dots,1)$ may run out of the cone. Therefore,
