...
\end{tikzpicture}
\end{center}
We first find a red point according to the hyperplain $z^1=z^3$
\[
	q=n_1+n_2,
\]
here we may need to rescale them such that coefficients are integers. 
...
\biggr\}
\] 
by blowing up. These matrices can be reduced to
\[
	M\mapsto \biggl\{
	\begin{pmatrix}
		0&1\\
...
It's easy to see that all new cones contain $(1,1)$, so we can blow up them repeatly 
and get
\begin{align*}
	M&\mapsto 
	\biggl\{
...
corresponding to $M$, we should carefully add factors $1/c_i$
corresponding to the change of variabes $x_i\mapsto x_i^{c_i}$ and we will get $5$ terms 
in above example.
\begin{align*}
...
\frac{1}{X (-2 c+X+Y) (-c+X+Z)}+\frac{1}{Z (-2 c+X+Y) (-c+X+Z)}\\
	&+\frac{1}{Z (-2 c+X+Y) (-3 c+X+2 Y)}+2\biggl(
\frac{1}{Z (-3 c+X+2 Y) (-2 c+2 Y+Z)}\\
	&+\frac{1}{2 Y (-3 c+X+2 Y) (-2 c+2 Y+Z)}
\biggr)\biggr)
+O({\alpha'}^{-2}).
\end{align*}
