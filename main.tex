...
Section 3 uses several examples arising form cluster stringy integrals to illustrate this algorithm.
In section 4, we introduce a new viewpoint to approach the Hironaka's polyhedra game
...
by $t$ with an additional point $[-b_i:a_i]\in \mathbb P^1$. The additional point $[-b_i:a_i]\in \mathbb P^1$ tells us how this line goes to $(0,0)$ so that it can be used to distinguish different lines.
...
For the second example \eqref{exam:2}, we 
...
\begin{figure}[th]
	\centering\includegraphics[scale=0.7]{fig_2.pdf}	
	\caption{
		Blue curve: $x^3+y^3+xy=0$; Green curve: $y+x^2=0$;
		Orange curve $x+y^2=0$.
	}
\end{figure}
...
\]
if we find $x^{n^J}$ and $x^{n^I}$ where
...
\text{$v_i\geq 0$ for all $i$}\}$, the above
