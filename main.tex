...
Now let's consider the general story. Generally, the integral near a vertex need not to be decoupled as $I(0,\infty)$ above, so blowing-up is necessary. One important feature of $\Omega_{\mathcal P}$ is `invariant' under the blowing-up because the residue of the canonical form on the boundary is the canonical form of the boundary.
...
Suppose we are at the vertices defined by $x_1,\dots,x_D=0$, and for each polynomial $p(x)=\sum_{I\in \mathscr A}a_I x^{n^I}$ we can choose a positive basis of $\{n^I\}_{I\in \mathscr B}$ such that for all $I \in \mathscr A$, 
\[
	n^I = \sum_{J\in \mathscr B}c^I_J n^J,
\]
where $c^I_J\geq 0$ are rational numbers, and then define a new polynomial $\bar p(x)=\sum_{J\in \mathscr B}a_J x^{n^J}$. For example, for $p=x+y^2+xy$, $\bar p=x+y^2$.
