...
\]
This can be visualized by the polyhedron. For instance, the Newton polyhedron of $p(x)=x^3+xy+y^3$ is the gray polyhedron, we first get the green polyhedron by $ y\mapsto xy$, but it's not the wanted (decoupled) polyhedron, so we blow up it by $y\mapsto xy$ again and we get the red polyhedron.
\begin{center}
