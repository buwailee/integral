...
Now come back to the step (3). One possible way to `divide' the cone 
is to reduce the number of outside vertices. Let's use a $N=3$ example to 
explain it.
\begin{center}
	\begin{tikzpicture}
		...
		\draw[white] (0.6,-4.5) -- (5.6,-3.5);
		\node[white] at (0.4,-1.7) {\text{Inside}};
		\node[white] at (0.7,-3.1) {\text{Outside}};
	\end{tikzpicture}
\end{center}
The outside vertices are labeled by blue points, and insider vertices are
labeled by red points.
