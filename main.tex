...
In section 2, we review the defination of blow up and elaborate . In section 3, we. Section 4 contains . Finally, in appendix, we will give another proof of the proposition firstly given in \cite{} that the scattering-equation map \eqref{SEmap} from the saddle point equations of the integral eq.\eqref{int1} 
...
\begin{enumerate}
	...
	% \item Usually, $(1,\dots,1)$ is not in the cone spanned by row vectors $\{n_i\,:\,i\in S\}$. 
	% What's more, if $(1,\dots,1)$ is not in this cone, then any cone generated
...
\begin{enumerate}
	...
	\item Find the minimal cone contains $(1,\dots,1)$, blow up according to
		the set of variables corresponding to its edges. Repeat it until no cone 
		contains $(1,\dots,1)$.
	\item For a given matrix, find all edges of its cone, and 
		divide the cone along the other vectors which are not corresponding to edges
		by blowing up.
	\item After the above step, there's no extra vector in the cone, though we could have 
	...
	\item After the above step, the number of columns of all matrices strictly decrease,
...
\draw[thick,dashed] (0,4.5) -- (2.667,2.667) node[circle, fill=red, inner sep=1pt] {}
	-- (2.12,2.12) node[circle, fill=red, inner sep=1pt] {};
\node[red] at (3,2.6) {$p_1$};
\node[red] at (2.1,1.8) {$p_2$};
\end{tikzpicture}	
\end{center}
We can ...

\[
	p_1=n_2+2n_3,\quad p_2=2 n_1+3 n_2,
\] 
so ... 
\[
	M\mapsto \biggl\{
...
Empirically, we have another way to choose $S$ for player A, which is inspired by 
above algorithm.
For a given matrix, if player A chooses $S$ such that 
\begin{compactenum}[\quad\, (1)]
\item $\sum_{i\in S} n_i^I>0$ for all $I$;
\item $\sum_{i\in S} n_i^I$ has maximal number of minimals;
\item $\#S$ is minimal,
\end{compactenum}
then he/she can still win in all cases we have tested. 
It's not clear now whether it is a 
winning strategy, but it's more efficient than any well-known winning strategy, and it's 
very easy to code.
