...
A is to find a set of variables to blow up at each turn such that $|M_p|$ strictly decreases.
...
In our code, We consider the following choice:

Write down the matrix $(n^I_i)$, and find the minimal set $S$ such that
\[
	\sum_{i\in S} n^I_i>0
\]
and 
\[
	g(S)=\max_I \biggl(\sum_{i\in S} n^I_i\biggr) - 
	\min_I \biggl(\sum_{i\in S} n^I_i\biggr)
\]
takes its minimum.
