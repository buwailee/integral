...
\textit{i.e.} the smallest convex set (\textcolor{red}{C: What is the smallest convex set?}) contains these cones.
\end{defi}
...
the cone (\textcolor{red}{C: Is Newton polyhedron more appropriate?}) is not as rigid as the Newton polytope, and it just looks like a `corner' of the Newton polytope.
...
In fact, if $x^{n^I}$ dominates $p$ in the integral region $\mathcal L$, 
then the contribution of integral from this region is
\begin{align*}
	\int_{\mathcal L} 
	\biggl(\prod_{i=1}^D\frac{d x_i}{x_i}x_i^{\alpha' X_i}\biggr)
	p(x)^{-\alpha'c}\prod_a(q_a(x))^{-\alpha' c_a}
...
\end{tikzpicture}
\end{center}
then we call that $p\approx x^v$ is \emph{decoupled}. In other words, $p$ has the form of %
$p(x)=x^v(\text{const.}+q(x)) $ for a vector $v$ and polynomial $q$ with $q(0)=0$. 
...
Since blow-ups never increase the number of polynomials in the integral, we only need to consider the cases that only contain one 
polynomial
\[
	I=\int_{[0,\epsilon]^D} \left(\prod_{i=1}^D\frac{\dif x_i}{x_i}x_i^{\alpha' X_i}\right)
	p^{-\alpha' c}.
\] 
Our aim is to find a series of blow-ups to make $p$ decoupled at each generated vertices.
...
is the `invariance' under blowing up, since the residue of the canonical form on the boundary is
...
change the variable $x_i\mapsto 1/x_i$. Let $x_i=ty_i$, where $[y_1:\dots:y_n]$ are positive projective coordinates. 
