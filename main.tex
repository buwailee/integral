...
For the remaining terms $x+y$, we can introduce new variables $x = tp$ and $y=t(1-p)$, 
\textit{i.e. blow-up}, then
...
\begin{enumerate}
	...
	It's a proper sub-cone of the original cone. The union of these cones is the original cone,
	but they usually intersect with each other, so it's not a subdivision. However, 
	it is a subdivision when $D\leq N$. For example, consider a cone with $D=N=3$,
	\begin{center}
	\begin{tikzpicture}
	\draw (1.7,2.6) -- (-2,-0.5);
	\draw (-2,-0.5) -- (3.5,-0.5);
	...
	\item If we can cut the old cone by blowing up into small cones such that 
	...
	the intersection of all hyperplains $z^I=z^J$, or equivalently the vector $(1,1,\dots,1)$, 
...
\begin{enumerate}
	...
	\item[(2).] Find the minimal cone containing $(1,\dots,1)$, blow up according to
	...
	\item[(3).] For each cone, choose vectors on some hyperplains $h^{IJ}=\{z\,:\, z^I=z^J\}$,
		and `divide' the cone along these vectors on hyperplains by blowing up such that 
...
In this article, we have shown how to obtain the leading order contribution of stringy integrals by a blow-up algorithm. These integrals are generations of tree-level scattering of open strings and provide natural $\alpha'$-deformed cannonical forms for general polytopes.  Interestingly, the algorithm is equivalent to a winning strategy for a simplified version of Hironaka's polyhedra game since more moves can be taken in our case.
...
As we saw in section 2, lots of terms have been dropped during the process of blow-ups, while higher order contributions of such integrals with respect to $\alpha'$ certainly depend on the details of regulating polynoimals. One way to obtain the higher order contribution is to expand the integrand with respect to $\alpha'$ then integration, the obstacle to this expansion is singularities in poles. The blow-up procedure provide a way to remove this obstacle: each integration region producted by blow-ups only meets singularities of the canonical form at one vertex, then a subtractation can be easily made such that the integrand have no divergence in the integration region.
