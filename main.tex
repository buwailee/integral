...
\[
	...
	p(x)^{-c}\prod_a(q_a(x))^{-\alpha' c_a}.
\]
In the polynomial 
\[
	p(x)=\sum_{I} s_{I} x^{n^I}:=\sum_I s_{I} \prod_{i=1}^N x_i^{n^I_i},
\]
...
Geometrically, one can assign each term of $p$ with a cone 
...
\[
	...
	p_1^{-\alpha' c} \prod_a q_a^{-\alpha' c_I}
	\quad \text{and} \quad 
	I'=\int_{[0,\epsilon]^D} \left(\prod_{i=1}^D\frac{\dif x_i}{x_i}x_i^{\alpha' X_i}\right)
	p_2^{-\alpha' c} \prod_a q_a^{-\alpha' c_I}
\]
...
\textcolor{blue}{(B: It's wrong. We also need to prove that 
it doesn't depend on coefficients in polynomial.)}
