...
\begin{pro}
For an arbitrary product of $M$ polynomials $P=\prod_{j=1}^M p_j^{\alpha_j}$, 
where $p_j=\sum_{I_j} a_{I_j} x^{I_j}$ with $a_{I_j}\geq 0$ for all $I_j$ and
\[
...
Consider a polynomial $p=\sum_{I} a_I x^I$ with $a_I\geq 0$ for all $I$. The scattering map now becomes
\[
	\mathbf X(x)=\frac{\partial \log p}{\partial \log \mathbf x},
\]
...
To prove it, let $\mathbf{\Lambda}$ be an interior point in $N(p)$ defined by 
\[
...
We assume that $\alpha_p>0$ for all $p$. Here, $F(\mathbf y)$ no longer needs to be
